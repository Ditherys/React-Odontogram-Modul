...
export function Chart() {
  return (
    <OdontogramShell
      language="ar"          // hu | en | de | es | it | sk | pl | ru | pt-br | ar | zh
...
- 💾 تصدير/استيراد الحالة بصيغة JSON (الإصدار 2.19؛ لا تزال الاستيرادات تقبل الإصدارات القديمة 1.4 ومن 2.0 حتى 2.18 وتُرحَّل تلقائيًا، مع حالات مخصصة للإضافات وملاحظات لكل سن)
...
- 📝 ملاحظات لكل سن: نقر مزدوج لإضافة/تعديل الملاحظات، أيقونة ملاحظة بجانب رقم السن، تلميح عند المرور بالمؤشر يعرض نص الملاحظة، تصدير/استيراد JSON
...
- 🧪 نجاح 1704 اختبارًا آليًا (مع تخطي اختبار إضافي واحد) (Vitest) عبر 163 ملف اختبار تغطي الترقيم، والترجمات، والأنماط الجاهزة، والدعم متعدد اللغات، ومكوّن App، والسمة، واللمس، والإضافات، وإمكانية الوصول، وتكافؤ محاور التشخيص/المحاور السريرية
...
| `exportPdf(opts)` | تنزيل تقرير PDF أصلي عبر jsPDF (`{patientData, odontogram, perioStatus, perioDescription}`، كل قسم اختياري) — نص متجهي بالإضافة إلى صور نقطية للأسنان/مخطط دواعم السن؛ يُتخطى قسما دواعم السن تلقائيًا كلما كانت `hasAnyPerioData()` تساوي `false`، بصرف النظر عن `opts` |
...
ينشئ التصدير ملف JSON (الإصدار `2.19`؛ لا تزال الاستيرادات تقبل الإصدارات القديمة `1.4` ومن `2.0` حتى `2.18` وتُرحَّل تلقائيًا) يحتوي على:
...
- **تقرير PDF:** يفتح عنصر "PDF report…" في قائمة التصدير نافذة `ExportOptionsModal` — وهي حوار إعدادات (حقلا اسم المريض وتاريخ الفحص، مرتبطان مباشرة بالبيانات الوصفية للحالة؛ وأربع خانات لأقسام: بيانات المريض، الأودونتوغرام، حالة دواعم السن، وصف دواعم السن) قبل استدعاء `exportPdf(opts)`. يُجمَّع ملف PDF بشكل أصلي عبر jsPDF — نص متجهي بواسطة `.text()`، وصور نقطية للأسنان/مخطط دواعم السن بواسطة `.addImage()` — **دون الاعتماد على مكتبة svg2pdf.js**. يُتخطى قسما دواعم السن تلقائيًا كلما كانت `hasAnyPerioData()` تساوي `false`، بصرف النظر عن خانات الحوار.
...
- اسم المريض وتاريخ الفحص بيانات هوية للمخطط فقط (الحمولة `2.19`، إضافية) — وهما **ليسا** جزءًا من تصدير FHIR.
