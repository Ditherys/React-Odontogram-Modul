...
حفظ اختياري لحالة الحالة السريرية في `localStorage` (‏`src/persistence.ts`، يُعاد تصديره من نقطة دخول الحزمة). معطّل افتراضيًا — التكاملات القائمة لا تتأثر ما لم يقم التطبيق المضيف بتفعيله صراحةً، وينبغي استدعاؤه **بعد** أن يتم تركيب (mount) المخطط السني (تعيد الاستعادة رسم DOM الحي عبر `importStatus()`):
...
| الدالة | الوصف |
|---|---|
| `enablePersistence(options?)` | تستعيد حالة محفوظة سابقًا (إن وُجدت) عبر `importStatus()`، ثم تحفظ مخطط الحالة إلى `localStorage` عند كل تغيير في الحالة. متكررة الاستدعاء بأمان (idempotent) — استدعاؤها مرة أخرى يستبدل الاشتراك/الخيارات السابقة. **يجب استدعاؤها بعد تركيب المخطط السني.** |
