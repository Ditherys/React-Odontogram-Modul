...
describe("collectExportPayload — version 2.11, status-primary, conditional plan", () => {
  it("export in STATUS mode: version 2.11, teeth reflects status, NO plan key when plan is uninitialized", () => {
    ...
    expect(payload.version).toBe("2.11");
...
describe("getStatusChart / getPlanChart / setPlanChart — per-state API", () => {
  it("getStatusChart()/getPlanChart() return the two charts' payloads independently", () => {
    ...
    expect(statusPayload.version).toBe("2.11");
    expect(planPayload.version).toBe("2.11");
